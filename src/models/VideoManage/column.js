...
export function getColumns() {
    return [{
        ...
        render: (text, record, index) => {
            return (
                <Link to={`${urls.VIDEO_LIST}/VideoDetail/${record.videoBaseId}`} className={style.card}>
        ...
        render: (text, record, index) => {
            if (record.status === 0) { // 已保存未上架
                return (
                    <Link to={`${urls.VIDEO_LIST}/VideoDetail/${record.videoBaseId}`} className={style.card}>编辑</Link>
                );
                ...
                return <Link to={`${urls.VIDEO_LIST}/VideoDetail/${record.videoBaseId}`} className={style.card}>编辑</Link>;
            }
        },
    }];
}
