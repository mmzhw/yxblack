...
import ImageUpload from '../../../../components/Upload/indexTV';
...
class DetailComponent extends Component {
    constructor(props) {
        ...
        this.dxOriginalUrlChange = this.dxOriginalUrlChange.bind(this);
                ...
                if (res.code === 0) {
                    ...
                    this.setState({
                        ...
                        if (this.state.video.categories !== 2) { // 单视频
                            video.originalUrl = this.trim(res.data.vVideoDetailRspVos.length > 0 && res.data.vVideoDetailRspVos[0].originalUrl ? res.data.vVideoDetailRspVos[0].originalUrl : '');
                            this.setState({
                                video
                            });
                        }
    ...
    updateVideo(e) {
        ...
        this.props.form.validateFields((err, values) => {
            ...
            if (video.categories !== 2 && video.originalUrl) {
                videoToUpdate.originalUrl = this.trim(video.originalUrl);
            }
            if (video.categories !== 2 && video.dxOriginalUrl) {
                videoToUpdate.dxOriginalUrl = video.dxOriginalUrl;
            }
    ...
    handleEditSubmit() {
        this.props.form.validateFields((err, values) => {
            if (err) return;
            console.log('this.state.videothis.state.video');
            let video = this.state.video;
            let videoToUpdate = {
                videoBaseId: video.videoBaseId,
                name: video.name,
                    ...
                    <Form className={style['form']} onSubmit={this.updateVideo}>
                        ...
                        {(
                            this.state.video.categories !== 2 ? <InputRow
                                ...
                                formMethods = { this.props.form }
                                message = '请输入视频源'
                                onChangeHandle = {this.dxOriginalUrlChange}
                                initialValue = { this.state.video.dxOriginalUrl }
                            /> : ''
                        )}
                        <InputRow
                            id = 'name'
    ...
    dxOriginalUrlChange(dxOriginalUrl) {
        console.log('dxOriginalUrldxOriginalUrl', dxOriginalUrl);
        let video = this.state.video;
        video.dxOriginalUrl = dxOriginalUrl;
        this.setState({
            video
        });
    }
